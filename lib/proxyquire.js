...
Proxyquire.prototype.load = function (request, stubs) {
  ...
  require.extensions[extname] = function ext(module, filename) {
    ...
    var createRequestHandler = function (requireImpl) {
      return function (request) {
        ...
        if (!stubs.hasOwnProperty(request)) return requireImpl(request);

        var stub = stubs[request];

        if (stub.hasOwnProperty('@noCallThru') ? !stub['@noCallThru'] : !self._noCallThru)
          fillMissingKeys(stub, requireImpl(request));

        return stub;
      };
    };
    module.requirejs = createRequestHandler(requirejs);
    module.require = createRequestHandler(require_super);
